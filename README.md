...
Twelve tests against a throwaway in memory SQLite database. The ones worth reading are
...
Offline queueing on the client so edits made without signal sync when it comes back, and Alembic
migrations instead of `create_all` before this ever holds data anyone cares about.
